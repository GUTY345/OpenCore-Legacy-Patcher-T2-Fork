...
class HardwarePatchsetDetection:
    ...
    def _validation_check_filevault_is_enabled(self) -> bool:
        ...
        if nvram and "-allow_fv" in nvram:
            return False

        # SECURITY FIX: Use list-based arguments to prevent command injection
        result = subprocess.run(["/usr/bin/fdesetup", "status"], capture_output=True, text=True)
        return "FileVault is Off" not in result.stdout
    ...
    def _validation_check_force_opengl_missing(self) -> bool:
        ...
        if nv_on and "ngfxgl=" in nv_on:
            return False
        for gpu in self._constants.computer.gpus:
            if isinstance(gpu, device_probe.NVIDIA) and gpu.disable_metal:
                return False
        return True
    ...
    def _validation_check_force_compat_missing(self) -> bool:
        ...
        """
        nv_on = utilities.get_nvram("boot-args", decode=True)
        if nv_on and "ngfxcompat=" in nv_on:
            return False
        for gpu in self._constants.computer.gpus:
            if isinstance(gpu, device_probe.NVIDIA) and gpu.force_compatible:
                return False
        return True
    ...
    def _validation_check_nvda_drv_missing(self) -> bool:
        ...
        """
        nv_on = utilities.get_nvram("boot-args", decode=True)
        if nv_on and "nvda_drv_vrl=" in nv_on:
            return False
        if utilities.get_nvram("nvda_drv"):
            return False
        return True
    ...
    def _dortania_internal_check(self) -> bool:
        """
    ...
    def _can_patch(self, requirements: dict, ignore_keys: list[str] = []) -> bool:
        ...
        for key, value in requirements.items():
            if key in ignore_keys or not key.startswith("Validation:"):
                continue
            if value is True:
                return False
        return True
    ...
    def _strip_incompatible_hardware(self, present_hardware: list[BaseHardware]) -> list[BaseHardware]:
        """
        Strip out incompatible hardware. Priority is given to Metal GPUs.
        """
    ...
    def _handle_missing_network_connection(self, requirements: dict, device_properties: dict) -> tuple[dict, dict]:
        ...
        if self._already_has_networking_patches() or not any(key.startswith("Networking:") for key in device_properties):
            return requirements, device_properties

        for key in list(device_properties.keys()):
            if not key.startswith("Networking:"):
    ...
    def _handle_sip_breakdown(self, requirements: dict, required_sip_configs: list[str]) -> dict:
        ...
        if HardwarePatchsetValidation.SIP_ENABLED in requirements:
    ...
    def _detect(self) -> None:
        ...
        for hardware in self._hardware_variants:
            ...
            if item.present():
                # Only skip native hardware if we are NOT in validation mode
                if item.native_os() and not self._validation:
                    continue
                present_hardware.append(item)

        # SECURITY FIX: Strip incompatible hardware regardless of validation state
        ...
        requirements = {
            ...
            HardwarePatchsetValidation.WHATEVERGREEN_MISSING:       self._validation_check_whatevergreen_missing() if has_nvidia_web_drivers else False,
            HardwarePatchsetValidation.FORCE_OPENGL_MISSING:        self._validation_check_force_opengl_missing()  if has_nvidia_web_drivers else False,
            HardwarePatchsetValidation.FORCE_COMPAT_MISSING:        self._validation_check_force_compat_missing()  if has_nvidia_web_drivers else False,
            HardwarePatchsetValidation.NVDA_DRV_MISSING:            self._validation_check_nvda_drv_missing()      if has_nvidia_web_drivers else False,
        }

        if requirements[HardwarePatchsetValidation.SIP_ENABLED]:
        ...
        # Third pass to sync stripped hardware
        for item in present_hardware:
            if item.name() in device_properties:
        ...
        requirements[HardwarePatchsetValidation.UNPATCHING_NOT_POSSIBLE] = requirements[HardwarePatchsetValidation.SIP_ENABLED]

        # LOGIC FIX: Patching is only allowed if validation passes AND there are patches to apply
        self.can_patch   = (not _cant_patch) and (len(patches) > 0)
        self.can_unpatch = not requirements[HardwarePatchsetValidation.SIP_ENABLED]
    ...
    def detailed_errors(self) -> None:
        ...
        for key, value in self.device_properties.items():
            if key.startswith("Validation:") and value is True:
                if key not in [HardwarePatchsetValidation.PATCHING_NOT_POSSIBLE, HardwarePatchsetValidation.UNPATCHING_NOT_POSSIBLE]:
