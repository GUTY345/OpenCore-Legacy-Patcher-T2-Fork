...
class GenerateDefaults:
    ...
    def __init__(self, model: str, host_is_target: bool, global_constants: constants.Constants, ignore_settings_file: bool = False) -> None:
        ...
        # Populate device_properties with model-specific features (e.g., T2_CHIP)
        # This is used by build.py and security.py to apply T2-specific patches
        if not hasattr(self.constants, "device_properties"):
            self.constants.device_properties = {}
        if self.model not in self.constants.device_properties:
            self.constants.device_properties[self.model] = {"Features": []}
            if self.model in smbios_data.smbios_dictionary:
                entry = smbios_data.smbios_dictionary[self.model]
                # T2 Macs have a non-None SecureBootModel and are not Apple Silicon
                if entry.get("SecureBootModel") is not None and entry.get("CPU Generation") != cpu_data.CPUGen.apple_silicon.value:
                    self.constants.device_properties[self.model]["Features"].append("T2_CHIP")
