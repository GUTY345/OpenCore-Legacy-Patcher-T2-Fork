...
class BuildMiscellaneous:
    ...
    def _re_generate_patch_arguments(self) -> list:
        ...
        if self.model in ["MacBookPro15,1", "MacBookPro15,2", "MacBookPro15,3", "MacBookPro15,4"]:
            logging.info("- Enabling RestrictEvents sbvmm/cpuname patching for Coffee Lake T2 MacBookPro models")
            for patch in ["sbvmm", "cpuname"]:
                if patch not in re_patch_args:
                    re_patch_args.append(patch)
    ...
    def _t2_handling(self) -> None:
        ...
        """
        # Determine if this model has a T2 chip via device_properties or fallback to hardcoded list
        is_t2 = False
        if hasattr(self.constants, "device_properties"):
            if "T2_CHIP" in self.constants.device_properties.get(self.model, {}).get("Features", []):
                is_t2 = True
        # Fallback for compatibility if device_properties not yet populated
        ...
        _GUID = "7C436110-AB2A-4BBB-A880-FE41995C9F82"
        for _arg in ["-v", "rddelay=5", "amfi=0x80", "igfxfw=2", "igfxonln=1", "revpatch=sbvmm", "-vsmccpu=1"]:
            current = self.config["NVRAM"]["Add"][_GUID].get("boot-args", "")
            if _arg not in current:
                self.config["NVRAM"]["Add"][_GUID]["boot-args"] = (current.rstrip() + " " + _arg).strip()

        current = self.config["NVRAM"]["Add"][_GUID].get("boot-args", "")
        if "revpatch=sbvmm" in current and "revpatch=sbvmm,cpuname" not in current:
            self.config["NVRAM"]["Add"][_GUID]["boot-args"] = current.replace("revpatch=sbvmm", "revpatch=sbvmm,cpuname")

        logging.info("- Enabling VirtualSMC plugin support for T2 SMC stability")
        builder = support.BuildSupport(self.model, self.constants, self.config)
        for kext_name in ["VirtualSMC.kext", "SMCProcessor.kext", "SMCSuperIO.kext"]:
            try:
                builder.enable_kext(kext_name, "", self.constants.kexts_path)
            except Exception as e:
                logging.info(f"- Optional plugin {kext_name} unavailable or not present: {e}")
        builder.insert_kext_after("SMCProcessor.kext", "VirtualSMC.kext")
        builder.insert_kext_after("SMCSuperIO.kext", "VirtualSMC.kext")

        logging.info("-Adding vmmroot=1 arguments")
        _current = self.config["NVRAM"]["Add"][_GUID].get("boot-args", "")
        if "vmmroot=1" not in _current:
            self.config["NVRAM"]["Add"][_GUID]["boot-args"] = (_current.rstrip() + " vmmroot=1").strip()
        logging.info("-Set NVRAM variable to -disable_ext_panics")
        _current = self.config["NVRAM"]["Add"][_GUID].get("boot-args", "")
        if "-disable_ext_panics" not in _current:
            self.config["NVRAM"]["Add"][_GUID]["boot-args"] = (_current.rstrip() + " -disable_ext_panics").strip()
        ...
        if "Kernel" not in self.config:
            self.config["Kernel"] = {}
        if "Emulate" not in self.config["Kernel"]:
