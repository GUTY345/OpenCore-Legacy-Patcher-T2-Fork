...
class BuildSupport:
    ...
    def insert_kext_after(self, kext_name: str, anchor_kext_name: str) -> None:
        """
        Reorder a kext in Kernel -> Add so it is loaded after a given anchor kext.
        """
        try:
            kext = self.get_kext_by_bundle_path(kext_name)
            anchor = self.get_kext_by_bundle_path(anchor_kext_name)
            if kext in self.config["Kernel"]["Add"] and anchor in self.config["Kernel"]["Add"]:
                self.config["Kernel"]["Add"].remove(kext)
                self.config["Kernel"]["Add"].insert(self.config["Kernel"]["Add"].index(anchor) + 1, kext)
        except Exception:
            logging.info(f"- Could not reorder {kext_name} after {anchor_kext_name}; entry missing.")
