...
class BuildFirmware:
    ...
    def _apple_logo_handling(self) -> None:
        ...
        # Macs that natively support Monterey (excluding MacPro6,1, Macmini7,1) won't have boot.efi draw the Apple logo.
        # This causes a cosmetic issue when booting through OpenCore, as the Apple logo will be missing.

        if not self.model in smbios_data.smbios_dictionary:
            return

        if smbios_data.smbios_dictionary[self.model]["Max OS Supported"] >= os_data.os_data.monterey and self.model not in ["MacPro6,1", "Macmini7,1", "MacBookAir8,1", "MacBookAir8,2", "MacBookAir9,1", "MacBookPro15,1", "MacBookPro15,2", "MacBookPro15,3", "MacBookPro16,3", "Macmini8,1", "iMacPro1,1"]:
