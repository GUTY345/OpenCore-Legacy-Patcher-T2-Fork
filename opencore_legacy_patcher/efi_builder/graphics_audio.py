    ...
    def _spoof_handling(self) -> None:
        ...
        if self.model in model_array.DualGPUPatch:
            ...
            if self.model in model_array.IntelNvidiaDRM and self.constants.drm_support is True:
                ...
                "T2_CHIP" in self.constants.device_properties.get(self.model, {}).get("Features", [])
                or self.model in ["MacBookPro15,1", "MacBookPro15,2", "MacBookPro15,3", "MacBookPro15,4",
                                   "MacBookPro16,1", "MacBookPro16,3", "MacBookPro16,4",
                                   "MacBookAir8,1", "MacBookAir8,2", "MacBookAir9,1",
                                   "Macmini8,1", "iMacPro1,1"]
            ):
                # T2 MacBook Pro with AMD dGPU: apply AMD-specific patches for GPU switching
                if self.gfx0_path not in self.config["DeviceProperties"]["Add"]:
                    self.config["DeviceProperties"]["Add"][self.gfx0_path] = {}
                logging.info("- Applying T2 AMD dGPU switching patches")
                self.config["DeviceProperties"]["Add"][self.gfx0_path].update({
                    "agdpmod": "pikera",
                    "shikigva": 128,
                    "unfairgva": 1,
                    "rebuild-device-tree": 1,
